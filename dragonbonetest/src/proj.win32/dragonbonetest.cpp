    ...
    int       nCmdShow)
{
    ...
    return DragonbonetestPlayer::createAndRun();
}

int DragonbonetestPlayer::createAndRun(void)
{
    DragonbonetestPlayer *host = DragonbonetestPlayer::sharedInstance();
    int ret = host->run();
    DragonbonetestPlayer::purgeSharedInstance();
    return ret;
}

DragonbonetestPlayer *DragonbonetestPlayer::s_sharedInstance = NULL;

DragonbonetestPlayer *DragonbonetestPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new DragonbonetestPlayer();
    }
    return s_sharedInstance;
}

void DragonbonetestPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
DragonbonetestPlayer::DragonbonetestPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int DragonbonetestPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "dragonbonetest-player");
...
void DragonbonetestPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void DragonbonetestPlayer::createViewMenu(void)
{
...
void DragonbonetestPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void DragonbonetestPlayer::updateWindowTitle(void)
{
}

void DragonbonetestPlayer::relaunch(void)
{
...
void DragonbonetestPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void DragonbonetestPlayer::onFileOpenProject(void)
{
...
void DragonbonetestPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"DragonbonetestPlayer");
...
void DragonbonetestPlayer::onFileProjectConfig(void)
{
...
void DragonbonetestPlayer::onFileRelaunch(void)
{
    relaunch();
}

void DragonbonetestPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void DragonbonetestPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void DragonbonetestPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void DragonbonetestPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void DragonbonetestPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT DragonbonetestPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    DragonbonetestPlayer *host = DragonbonetestPlayer::sharedInstance();
...
INT_PTR CALLBACK DragonbonetestPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
