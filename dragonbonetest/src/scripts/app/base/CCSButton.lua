...
--    button:onButtonClicked(function(event)
	button:addTouchEventListener(function(object,event)
		if event == TOUCH_EVENT_BEGAN then --点击下去
			clickPressHandle(true);
		elseif event == TOUCH_EVENT_MOVED then --移动
		elseif event == TOUCH_EVENT_CANCELED then --当触发move后  已经移出到其他部位上去了   然后在up弹起
			clickPressHandle(false);
		elseif event == TOUCH_EVENT_ENDED then --点击放开后
			clickPressHandle(false);
			clickOneceHandle();
		end
